...
static int casadi_f0(const casadi_real** arg, casadi_real** res, casadi_int* iw, casadi_real* w, int mem) {
  ...
  *rr++ = w6;
  ...
  /* #73: @20 = all_2.22045e-16(3x1) */
  casadi_fill(w20, 3, 2.2204460492503131e-16);
  /* #74: @24 = zeros(4x1) */
  casadi_clear(w24, 4);
  ...
  *rr++ = w8;
  ...
  w4 = 2.;
  /* #115: (@23[4] = @4) */
  for (rr=w23+4, ss=(&w4); rr!=w23+5; rr+=1) *rr = *ss++;
  /* #116: @4 = 2 */
  w4 = 2.;
  /* #117: (@23[8] = @4) */
  for (rr=w23+8, ss=(&w4); rr!=w23+9; rr+=1) *rr = *ss++;
  /* #118: @1 = mac(@22,@23,@1) */
  for (i=0, rr=w1; i<3; ++i) for (j=0; j<1; ++j, ++rr) for (k=0, ss=w22+j, tt=w23+i*3; k<3; ++k) *rr += ss[k*1]**tt++;
  /* #119: @3 = mac(@1,@20,@3) */
  for (i=0, rr=(&w3); i<1; ++i) for (j=0; j<1; ++j, ++rr) for (k=0, ss=w1+j, tt=w20+i*3; k<3; ++k) *rr += ss[k*1]**tt++;
  /* #120: @2 = (@2*@3) */
  w2 *= w3;
  /* #121: @0 = (@0+@2) */
  w0 += w2;
  ...
  /* #125: @20 = @19[10:13] */
  for (rr=w20, ss=w19+10; ss!=w19+13; ss+=1) *rr++ = *ss;
  /* #126: @1 = (@1-@20) */
  for (i=0, rr=w1, cs=w20; i<3; ++i) (*rr++) -= (*cs++);
  /* #127: @20 = @1' */
  casadi_copy(w1, 3, w20);
  /* #128: @3 = mac(@20,@1,@3) */
  for (i=0, rr=(&w3); i<1; ++i) for (j=0; j<1; ++j, ++rr) for (k=0, ss=w20+j, tt=w1+i*3; k<3; ++k) *rr += ss[k*1]**tt++;
  /* #129: @2 = (@2*@3) */
  w2 *= w3;
  /* #130: @0 = (@0+@2) */
  w0 += w2;
  ...
  /* #134: @1 = @19[3:6] */
  for (rr=w1, ss=w19+3; ss!=w19+6; ss+=1) *rr++ = *ss;
  /* #135: @20 = (@20-@1) */
