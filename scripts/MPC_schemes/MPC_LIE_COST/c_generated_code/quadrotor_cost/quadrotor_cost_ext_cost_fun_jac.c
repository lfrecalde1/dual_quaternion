...
static int casadi_f0(const casadi_real** arg, casadi_real** res, casadi_int* iw, casadi_real* w, int mem) {
  ...
  *rr++ = w42;
  ...
  /* #175: @1 = mac(@46,@37,@1) */
  for (i=0, rr=(&w1); i<1; ++i) for (j=0; j<1; ++j, ++rr) for (k=0, ss=w46+j, tt=w37+i*3; k<3; ++k) *rr += ss[k*1]**tt++;
  /* #176: @0 = (@0+@1) */
  w0 += w1;
  /* #177: @1 = 0 */
  w1 = 0.;
  /* #178: @46 = @35[11:14] */
  for (rr=w46, ss=w35+11; ss!=w35+14; ss+=1) *rr++ = *ss;
  /* #179: @47 = @46' */
  casadi_copy(w46, 3, w47);
  /* #180: @1 = mac(@47,@46,@1) */
  for (i=0, rr=(&w1); i<1; ++i) for (j=0; j<1; ++j, ++rr) for (k=0, ss=w47+j, tt=w46+i*3; k<3; ++k) *rr += ss[k*1]**tt++;
  /* #181: @0 = (@0+@1) */
  w0 += w1;
  /* #182: output[0][0] = @0 */
  if (res[0]) res[0][0] = w0;
  ...
  /* #198: (@35[11:14] += @46) */
  for (rr=w35+11, ss=w46; rr!=w35+14; rr+=1) *rr += *ss++;
  /* #199: @37 = (2.*@37) */
  ...
  w40 = (- w40 );
  /* #228: (@48[3] += @40) */
  for (rr=w48+3, ss=(&w40); rr!=w48+4; rr+=1) *rr += *ss++;
  /* #229: @41 = (-@41) */
  w41 = (- w41 );
  /* #230: (@48[2] += @41) */
  for (rr=w48+2, ss=(&w41); rr!=w48+3; rr+=1) *rr += *ss++;
  /* #231: @0 = (-@0) */
  w0 = (- w0 );
  /* #232: (@48[1] += @0) */
  for (rr=w48+1, ss=(&w0); rr!=w48+2; rr+=1) *rr += *ss++;
  /* #233: (@48[0] += @39) */
  ...
  /* #238: @20 = @19' */
  for (i=0, rr=w20, cs=w19; i<4; ++i) for (j=0; j<4; ++j) rr[i+j*4] = *cs++;
  /* #239: {@18, @16, @13, @17} = horzsplit(@20) */
  casadi_copy(w20, 4, w18);
  casadi_copy(w20+4, 4, w16);
  casadi_copy(w20+8, 4, w13);
  casadi_copy(w20+12, 4, w17);
  /* #240: @17 = @17' */
  /* #241: {@39, @0, @41, @40} = horzsplit(@17) */
  w39 = w17[0];
  w0 = w17[1];
  w41 = w17[2];
  w40 = w17[3];
  /* #242: (@48[4] += @40) */
  for (rr=w48+4, ss=(&w40); rr!=w48+5; rr+=1) *rr += *ss++;
  /* #243: (@48[5] += @41) */
  for (rr=w48+5, ss=(&w41); rr!=w48+6; rr+=1) *rr += *ss++;
  /* #244: @0 = (-@0) */
  w0 = (- w0 );
  /* #245: (@48[6] += @0) */
  for (rr=w48+6, ss=(&w0); rr!=w48+7; rr+=1) *rr += *ss++;
  /* #246: (@48[7] += @39) */
  for (rr=w48+7, ss=(&w39); rr!=w48+8; rr+=1) *rr += *ss++;
  /* #247: @13 = @13' */
  /* #248: {@39, @0, @41, @40} = horzsplit(@13) */
  w39 = w13[0];
  w0 = w13[1];
  w41 = w13[2];
  w40 = w13[3];
  ...
  w40 = (- w40 );
  /* #250: (@48[5] += @40) */
  for (rr=w48+5, ss=(&w40); rr!=w48+6; rr+=1) *rr += *ss++;
  /* #251: (@48[4] += @41) */
  for (rr=w48+4, ss=(&w41); rr!=w48+5; rr+=1) *rr += *ss++;
  /* #252: (@48[7] += @0) */
  for (rr=w48+7, ss=(&w0); rr!=w48+8; rr+=1) *rr += *ss++;
  /* #253: (@48[6] += @39) */
  for (rr=w48+6, ss=(&w39); rr!=w48+7; rr+=1) *rr += *ss++;
  /* #254: @16 = @16' */
  /* #255: {@39, @0, @41, @40} = horzsplit(@16) */
  w39 = w16[0];
  w0 = w16[1];
  w41 = w16[2];
  w40 = w16[3];
  /* #256: (@48[6] += @40) */
  for (rr=w48+6, ss=(&w40); rr!=w48+7; rr+=1) *rr += *ss++;
  /* #257: @41 = (-@41) */
  w41 = (- w41 );
  /* #258: (@48[7] += @41) */
  for (rr=w48+7, ss=(&w41); rr!=w48+8; rr+=1) *rr += *ss++;
  /* #259: (@48[4] += @0) */
  for (rr=w48+4, ss=(&w0); rr!=w48+5; rr+=1) *rr += *ss++;
  /* #260: (@48[5] += @39) */
  for (rr=w48+5, ss=(&w39); rr!=w48+6; rr+=1) *rr += *ss++;
  /* #261: @18 = @18' */
  /* #262: {@39, @0, @41, @40} = horzsplit(@18) */
  w39 = w18[0];
  w0 = w18[1];
  w41 = w18[2];
  w40 = w18[3];
  ...
  w41 = (- w41 );
  /* #266: (@48[6] += @41) */
  for (rr=w48+6, ss=(&w41); rr!=w48+7; rr+=1) *rr += *ss++;
  /* #267: @0 = (-@0) */
  w0 = (- w0 );
  /* #268: (@48[5] += @0) */
  for (rr=w48+5, ss=(&w0); rr!=w48+6; rr+=1) *rr += *ss++;
  /* #269: (@48[4] += @39) */
  for (rr=w48+4, ss=(&w39); rr!=w48+5; rr+=1) *rr += *ss++;
  /* #270: @39 = (@1/@9) */
  w39  = (w1/w9);
  /* #271: @10 = (@10*@39) */
  w10 *= w39;
  ...
  w29 *= w10;
  /* #275: (@48[2] += @29) */
  for (rr=w48+2, ss=(&w29); rr!=w48+3; rr+=1) *rr += *ss++;
  /* #276: @29 = (@38/@9) */
  w29  = (w38/w9);
  /* #277: @14 = (@14*@29) */
  w14 *= w29;
  /* #278: (@48[1] += @14) */
  for (rr=w48+1, ss=(&w14); rr!=w48+2; rr+=1) *rr += *ss++;
  /* #279: @14 = sq(@9) */
  w14 = casadi_sq( w9 );
  /* #280: @0 = sq(@12) */
  w0 = casadi_sq( w12 );
  ...
  w0 = (- w0 );
  /* #293: (@48[0] += @0) */
  for (rr=w48+0, ss=(&w0); rr!=w48+1; rr+=1) *rr += *ss++;
  /* #294: @31 = (@31/@9) */
  w31 /= w9;
  /* #295: @31 = (@31*@3) */
  w31 *= w3;
  ...
  /* #309: @2 = (@6*@48) */
  for (i=0, rr=w2, cs=w48; i<8; ++i) (*rr++)  = (w6*(*cs++));
  /* #310: @6 = 1 */
  w6 = 1.;
  /* #311: @15 = (@15?@6:0) */
  w15  = (w15?w6:0);
  ...
  /* #316: {@15, @6, @31, @9, @12, @32, @14, @27, @38, @34, @1, @3, @0, @4} = vertsplit(@35) */
  w15 = w35[0];
  w6 = w35[1];
  w31 = w35[2];
  w9 = w35[3];
