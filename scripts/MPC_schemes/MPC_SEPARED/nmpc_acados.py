...
def create_ocp_solver(x0, N_horizon, t_horizon, F_max, F_min, tau_1_max, tau_1_min, tau_2_max, tau_2_min, tau_3_max, tau_3_min,  L, ts)->AcadosOcp:
    ...
    ocp.model.cost_expr_ext_cost = error_pos.T@Q@error_pos + 1*(error_nominal_input.T @ R @ error_nominal_input) + 10*(error_ori.T@error_ori)

    ocp.model.cost_expr_ext_cost_e =  error_pos.T@Q@error_pos + 1*(w.T@w) + 10*(error_ori.T@error_ori)+ 1.0*(v.T@v)
