...
from ode_acados import dual_velocity_casadi, quat_multi_casadi, dualquat_quat_casadi, dualquat_trans_casadi, compute_reference, velocities_from_twist_casadi
from ode_acados import dualquat_get_real_casadi, dualquat_get_dual_casadi, rotation_casadi, rotation_inverse_casadi
...
rot_inertial = rotation_casadi()
rot_body = rotation_inverse_casadi()
velocities_from_twist = velocities_from_twist_casadi()


import os
script_dir = os.path.dirname(__file__)
folder_path = os.path.join(script_dir, 'results_body/')
...
def set_odom_msg(msg, x):
    ...
    t = get_trans(x)
    q = get_quat(x)
    t_i = rot_inertial(q, t[1:4])
    msg.header.stamp = rospy.Time.now()
    msg.header.frame_id = "world"

    msg.pose.pose.position.x = t_i[0, 0]
    msg.pose.pose.position.y = t_i[1, 0]
    msg.pose.pose.position.z = t_i[2, 0]
...
def main(ts: float, t_f: float, t_N: float, L: list, odom_pub_1, odom_pub_2, initial)-> None:
   ...
   # Odometry message 
    ...
    plot_states_velocity_reference(fig13, ax13, ax23, ax33, u[0:3, :], X_d[8:11, :], t, "Angular Velocity Body Frame of the System Based on LieAlgebra and Reference "+ str(initial), folder_path)

    fig15, ax15, ax25, ax35 = fancy_plots_3()
    plot_states_velocity_reference(fig15, ax15, ax25, ax35, u[3:6, :], X_d[11:14, :], t, "Linear Velocity Body Frame of the System Based on LieAlgebra and Reference "+ str(initial), folder_path)
