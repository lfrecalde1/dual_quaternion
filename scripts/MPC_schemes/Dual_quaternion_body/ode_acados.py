...
def dual_velocity_casadi(twist = w_1d, dualquat = dual_1d):
    ...
    t_aux = get_trans(dualquat)

    real = w
    dual = ca.cross(w, t_aux[1:4]) + v
...
def velocities_from_twist_casadi(twist = w_1d, dualquat = dual_1d):
    ...
    w_body = w
    v_body = dual - ca.cross(w, t_aux[1:4])

    velocity = ca.vertcat(w_body, v_body)
    f_velocity = Function('f_velocity', [w_1d, dual_1d], [velocity])
    return f_velocity
...
def ln_dual(q_error):
    ...
    ## Dual Part
    H_error_real_c_plus = ca.vertcat(ca.horzcat(q_error_real_c[0, 0], -q_error_real_c[1, 0], -q_error_real_c[2, 0], -q_error_real_c[3, 0]),
    ...
    trans_error = 2 * H_error_real_c_plus@q_error_dual
    # Computing log map
