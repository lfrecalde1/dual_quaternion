...
def plot_control_actions_angular(fig11, ax21, ax31, ax41, M, t, name, path):
        t = t[0:M.shape[1]]
        ax21.set_xlim((t[0], t[-1]))
        ax31.set_xlim((t[0], t[-1]))
        ax41.set_xlim((t[0], t[-1]))
...
def plot_cost_orientation_rotational(fig11, ax11, x_sample_real, t, name, path):
        t = t[0:x_sample_real.shape[1]]
        ax11.set_xlim((t[0], t[-1]))
        ...
        ax11.grid(color='#949494', linestyle='-.', linewidth=0.5)
        ax11.set_xlabel(r"$\textrm{Time}[s]$", labelpad=5)
        ax11.set_ylabel(r'$||Log{(\mathbf{R}^{T}_d \mathbf{R}})||^{2}$', labelpad=5)
...
def plot_cost_orientation_quat(fig11, ax11, x_sample_real, t, name, path):
        t = t[0:x_sample_real.shape[1]]
        ax11.set_xlim((t[0], t[-1]))
