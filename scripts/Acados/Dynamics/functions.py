...
def rotation():
    ...
    quat = Quaternion(q = quat_aux_1)
    quat_c = quat.conjugate()
    t = Quaternion(q = vector)

    t_new = quat * t * quat_c
    aux = t_new.get[1:4, 0]
    f_rot =  ca.Function('f_rot', [quat_aux_1, vector_aux_1], [aux])
    return f_rot
...
def velocities_body_from_twist_casadi(dual_velocity = W1d, Q_current = Q1d, W_data = w_1d_data, dual_data = dual_1d_data):
    ...
    v_i = dual - w_body.cross(p)
    v_body = quat_c * v_i * quat
    #v_body =  v_i 


    dual_velocity_aux = DualQuaternion_body(q_real = w_body, q_dual = v_body)
    aux = dual_velocity_aux.get[:, 0]
    velocity = ca.vertcat(aux[1, 0], aux[2, 0], aux[3, 0], aux[5, 0], aux[6, 0], aux[7, 0])
    f_velocity = Function('f_velocity', [W_data, dual_data], [velocity])
    return f_velocity

def velocities_from_twist_casadi(dual_velocity = W1d, Q_current = Q1d, W_data = w_1d_data, dual_data = dual_1d_data):
    # Get Real and dual values 
    w_body = dual_velocity.get_real
    dual = dual_velocity.get_dual
...
def dual_aceleraction_casadi(W = w_1d, dual = dual_1d, force = F, torques = tau):
    ...
    a = f_cross(-J_1@w, J@w)
    F_r = a
    F_d = f_cross(a, p) + f_cross(w, v) - g*e3

    # Compute forced part
    U_r = J_1@torques
    U_d = (force/m)*f_rotacion(q, e3) + f_cross(J_1@torques, p)
...
def lyapunov_casadi_simple(qd = dual_1d, q =dual_1, qd_data = dual_1d_data, q_data = dual_1_data):
    ...
    quat_data = qd_conjugate[0:4, 0]
    dual_data =  qd_conjugate[4:8, 0]
    ...
    # Real Part
    norm = ca.norm_2(q_error_real[1:4, 0] + ca.np.finfo(np.float64).eps)
    angle = ca.atan2(norm, q_error_real[0, 0])
...
def optimization_casadi(N, ts):
    ...
    for k in range(N):
        ...
        dual_velocity = dual(con, st)
        X[:,k+1] = f(st, dual_velocity, ts)
