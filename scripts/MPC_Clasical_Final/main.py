...
def main(ts: float, t_f: float, t_N: float, x_0: np.ndarray, L: list, odom_pub_1, odom_pub_2, pub_ref,  v_max, a_max, n, initial)-> None:
    # DESCRIPTION
    ...
    for k in range(0, t.shape[0] - N_prediction):
        ...
        marker_msg, points = send_marker(marker_msg, points, pub_ref, hd[0:, k])
...
if __name__ == '__main__':
    try: #################################### Simulation  #####################################################
        ...
        for k in range(0, X_total.shape[0]):
            # Simulation
            x, xref, F, M, orientation_cost, translation_cost, control_cost, t, N_prediction, kkt_values, sqp_iteration = main(ts, t_f, t_N, X_total[k, :], L, odometry_publisher_1, odometry_publisher_2, ref_publisher, combinations[k, 0], combinations[k, 1], 1, k)
