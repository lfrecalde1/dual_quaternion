...
subplot(3,2,1) ; grid minor; hold on; ylabel("$~^i \tilde{r}$",Interpreter="latex",FontSize=20,Rotation=90);
subplot(3,2,3) ; grid minor; hold on; ylabel("$~^i \tilde{\theta}$",Interpreter="latex",FontSize=20,Rotation=90)
subplot(3,2,5) ; grid minor; hold on; ylabel("$^{b}{\upsilon}_{xd}-~^b{\upsilon_{x}}$",Interpreter="latex",FontSize=20,Rotation=90);xlabel('$\textrm{Time}[s]$',Interpreter='latex')
...
%% sombreado zona transitoria y estable

min_r = min(find(std_r<mean_total_r));
min_t = min(find(std_t<mean_total_t));
min_v = min(find(std_v<mean_total_v));
% 
% subplot(6,2,4);hold on; 
% fill_1 = fill([ 0 0 min_r min_r ], [-0.05 0.4 0.4 -0.05], [255, 0, 0]/255);
% fill_1.FaceAlpha = 0.15;
% fill_1.EdgeColor = 'none';
% fill_2 = fill([ min_r min_r 800 800 ], [-0.05 0.4 0.4 -0.05], [0, 255, 0]/255);
% fill_2.FaceAlpha = 0.15;
% fill_2.EdgeColor = 'none';
% hold off; 
% 
% subplot(6,2,8);hold on; 
% fill_1 = fill([ 0 0 min_t min_t ], [-0.05 0.5 0.5 -0.05], [255, 0, 0]/255);
% fill_1.FaceAlpha = 0.15;
% fill_1.EdgeColor = 'none';
% fill_2 = fill([ min_t min_t 800 800 ], [-0.05 0.5 0.5 -0.05], [0, 255, 0]/255);
% fill_2.FaceAlpha = 0.15;
% fill_2.EdgeColor = 'none';
% hold off; 
...
% fill_1 = fill([ 0 0 min_v min_v ], [-0.015 0.6 0.6 -0.015], [255, 0, 0]/255);
% fill_1.FaceAlpha = 0.15;
% fill_1.EdgeColor = 'none';
% fill_2 = fill([ min_v min_v 800 800 ], [-0.015 0.6 0.6 -0.015], [0, 255, 0]/255);
% fill_2.FaceAlpha = 0.15;
% fill_2.EdgeColor = 'none';
% hold off; 


% subplot(3,2,2) ; h = boxplot(box_r(2:end)) ;%ol = findobj(h,'Tag','Outliers'); set(ol,'Visible','off');
% subplot(3,2,4) ; h = boxplot(box_t(2:end)) ;%ol = findobj(h,'Tag','Outliers'); set(ol,'Visible','off');
% subplot(3,2,6) ; h = boxplot(box_v(2:end)) ;%ol = findobj(h,'Tag','Outliers'); set(ol,'Visible','off');


%% limites graficas
subplot(3,2,1) ;xticks(0:5:40);xticklabels({'','','','','','','',''}); xlim([0 40]); 
subplot(3,2,3) ;xticks(0:5:40);xticklabels({'','','','','','','',''}); xlim([0 40]);
subplot(3,2,5) ;xticks(0:5:40);xlim([0 40]);

subplot(6,2,4) ;xticks(0:100:800);xticklabels({'','','','','','','',''}); xlim([0 800]); ylim([-0.05 0.4]); 
subplot(6,2,8) ;xticks(0:100:800);xticklabels({'','','','','','','',''}); xlim([0 800]); ylim([-0.05 0.5]); 
subplot(6,2,12) ;xticks(0:100:800);xticklabels({'0','5','10','15','20','25','30','35','40'}); xlim([0 800]); ylim([-0.015 0.6]); 

subplot(6,2,2) ;xticks(0:100:800);xticklabels({'','','','','','','',''}); xlim([0 800]); 
subplot(6,2,6) ;xticks(0:100:800);xticklabels({'','','','','','','',''}); xlim([0 800]); 
subplot(6,2,10) ;xticks(0:100:800);xticklabels({'0','5','10','15','20','25','30','35','40'}); xlim([0 800]);



%% Datos errores totales con su std
