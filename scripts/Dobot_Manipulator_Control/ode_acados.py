...
def Ad_aux_casadi():
    qd = ca.MX.sym('qd', 8, 1)
    v = ca.MX.sym('v', 6, 1)
    qd_normal = ca.vertcat(qd[0], qd[1], qd[2], qd[3], qd[4], qd[5], qd[6], qd[7])
    qd_conjugate = ca.vertcat(qd[0], -qd[1], -qd[2], -qd[3], qd[4], -qd[5], -qd[6], -qd[7])
    quat_d_data = qd_normal[0:4]
    dual_d_data =  qd_normal[4:8]
    ...
    vector_b_dual = Haux_plus@qd_conjugate
    vector_b = ca.vertcat(vector_b_dual[1:4], vector_b_dual[5:8])
    f_ad = Function('f_ad', [qd, v], [vector_b])
    return f_ad

def conjugate_dual_aux_casadi():
    qd = ca.MX.sym('qd', 8, 1)
    qd_conjugate = ca.vertcat(qd[0], -qd[1], -qd[2], -qd[3], qd[4], -qd[5], -qd[6], -qd[7])
    f_conjugate = Function('f_conjugate', [qd], [qd_conjugate])
    return f_conjugate

def dual_multiply_aux_casadi():
    qd = ca.MX.sym('qd', 8, 1)
    q = ca.MX.sym('q', 8, 1)
    qd_conjugate = ca.vertcat(qd[0], qd[1], qd[2], qd[3], qd[4], qd[5], qd[6], qd[7])
    quat_d_data = qd_conjugate[0:4]
...
def quadrotorModel()-> AcadosModel:
    ...
    X = ca.vertcat(qw_1d, qx_1d, qy_1d, qz_1d, dw_1d, dx_1d, dy_1d, dz_1d, theta_1, theta_2, theta_3, theta_4, theta_5, theta_6)
    ...
    X_dot = ca.vertcat(qw_1dot, qx_1dot, qy_1dot, qz_1dot, dw_1dot, dx_1dot, dy_1dot, dz_1dot, theta_1_dot, theta_2_dot, theta_3_dot, theta_4_dot, theta_5_dot, theta_6_dot)
    ...
    u = ca.vertcat(theta_1_c, theta_2_c, theta_3_c, theta_4_c, theta_5_c, theta_6_c)
